...
def new_task_post(router_id):
    computer_protocol.wait_for_work(int(router_id), str(request.form['task_id']), str(request.form['datahash']), str(request.form['key_fragments']), str(request.form['data_link']))
